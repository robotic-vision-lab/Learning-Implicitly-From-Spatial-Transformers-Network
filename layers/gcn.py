...
class TreeGCN(nn.Module):
    ...
    def init_param(self):
        if self.upsample:
            ...
            init.kaiming_normal_(self.W_branch.data, a = 0.2, mode='fan_in', nonlinearity = 'leaky_relu')
    ...
    def forward(self, tree):
        root = 0
        batch = tree[-1].size(0)    # dynamic batch allocation to test with differnt batch size
